...
# NN Simulation
def train(nHelperPoints, t_real, sol_noisy, nColloc, y_0, hasWeights):
    model = MT_nn()
    # Optimizer
...
def worker(scenarioId, nPoints, noiseLevels):
    ...
    for noise in noiseLevels:
        ...
        for point in nPoints:
            ...
            modelBalanced, trainingTime, idHPoints = train(point, t_real, solNoisy, 50, y_0, True)

            predBalanced = modelBalanced(t_real).detach().numpy()
            errorX = np.sqrt(np.mean((predBalanced[:, 0] - sol[:, 0])**2))
            errorY = np.sqrt(np.mean((predBalanced[:, 1] - sol[:, 1])**2))
            errorZ = np.sqrt(np.mean((predBalanced[:, 2] - sol[:, 2])**2))
            ...
            modelUnbalanced, trainingTime, idHPoints = train(point, t_real, solNoisy, 50, y_0, False)
            predUnbalanced = modelUnbalanced(t_real).detach().numpy()
            errorX = np.sqrt(np.mean((predUnbalanced[:, 0] - sol[:, 0])**2))
            errorY = np.sqrt(np.mean((predUnbalanced[:, 1] - sol[:, 1])**2))
            errorZ = np.sqrt(np.mean((predUnbalanced[:, 2] - sol[:, 2])**2))
...
# Initial conditions
x_0, y_0, z_0 = 0.95, 0.05, 0.0
y_0 = np.array([x_0, y_0, z_0])
...
# Real RK4 Solution
sol = rk4(maki_thompson, y_0, t_real.detach().numpy(), lambd, alpha)
# Learning Rate
lr = 1e-3
...
if __name__ == "__main__":
    ...
    for j in range (10):
        ...
        print(df)
        fileName = f"MT_results_{j}_NO_R0.csv" 
